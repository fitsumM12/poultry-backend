...
#     broilers = broilersDetail.objects.filter(farm_institution_id=pk)
#     serializer = BroilersDetailSerializer(broilers, many=True)
#     return Response(serializer.data)
...
#     serializer = BroilersDetailSerializer(data=request.data,context={'request': request})
#     if serializer.is_valid():
...
#         broiler_counts = (
#             broilersDetail.objects
#             .values('farm__institution__name')  
#             .annotate(broiler_count=Count('id'))
#             .order_by('farm_institution__name')
#         )

#         response_data = [
#             {
#                 "institution": data['farm__institution__name'],
...
from .models import broilersDetail, broilersImageAndPrediction
from .serializers import *
...
@permission_classes([IsAuthenticated])
def fetch_broilers_supervisor_api(request, pk):
    # ✅ FIXED (farm -> institution)
    broilers = broilersDetail.objects.filter(farm__institution_id=pk)
    serializer = BroilersDetailSerializer(broilers, many=True)
    return Response(serializer.data)
...
@permission_classes([IsAuthenticated])
def add_broiler_api(request):
    serializer = BroilersDetailSerializer(
        data=request.data,
    ...
    if serializer.is_valid():
        broiler = serializer.save()
        return Response({'id': broiler.id, **serializer.data}, status=201)
...
@permission_classes([IsAuthenticated])
def upload_raw_generate_cam_and_predictions(request):
    print("FILELS:", request.FILES)
    print("DATA:", request.data)
    broiler_id = request.data.get('broiler_id')
    if not broiler_id:
        return Response({'error': 'No broiler_id provided'}, status=400)
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def add_image_predictions(request):
    serializer = BroilersImageAndPredictionSerializer(data=request.data)
    if serializer.is_valid():
        broiler_health_status = serializer.save()
        return Response({'id': broiler_health_status.id}, status=201)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_latest_prediction(request, broiler_id):
    try:
        prediction = broilersImageAndPrediction.objects.filter(broiler_id=broiler_id).latest('id')
        serializer = BroilersImageAndPredictionSerializer(prediction)
        return Response(serializer.data)
    except broilersImageAndPrediction.DoesNotExist:
        return Response({'error': 'No prediction found'}, status=404)
...
def broiler_count_per_institution(request):
    """
    ✅ FIXED:
    uses farm__institution__name (correct relation)
    """
    ...
    broiler_counts = (
        ...
        .values('farm__institution__name')
        .annotate(broiler_count=Count('id'))
        .order_by('farm__institution__name')
    )

    response_data = [
        {
            "institution": row['farm__institution__name'],
            "broiler_count": row['broiler_count']
        }
        for row in broiler_counts
    ]
